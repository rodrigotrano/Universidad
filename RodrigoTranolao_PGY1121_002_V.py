...
def grabar_personas(personas):
    try:
        ...
        fecha_nacimiento = input("Ingrese la fecha de nacimiento de la persona (formato: YYYY-MM-DD): ")
        ...
        if index < 0:
            raise ValueError("El NIF debe tener el siguiente formato: 03034567-XXY")
        if edad < 0:
            raise ValueError("La edad debe ser mayor a 0. Verifique la fecha de nacimiento")
        if largoNombres < 8:
            raise ValueError("el valor nombres a registrar debe ser mayor a 8 caracteres. Intente nuevamente.")
        personas[nif] = {
            "nif":nif,            
            "nombres": nombres,
